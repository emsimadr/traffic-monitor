...
class TrackedVehicle:
    ...
    class_id: Optional[int] = None  # COCO class ID from detection
...
class VehicleTracker:
    ...
    def update(self, detections: np.ndarray, counting_line: Optional[List[Tuple[int, int]]] = None, 
               detection_metadata: Optional[List[Dict]] = None) -> List:
        """
        ...
        Args:
            ...
            detection_metadata: Optional list of dicts with class_id, class_name, confidence
            
        Returns:
            Empty list (counting is done by GateCounter, not tracker)
        """
        # Update existing tracked vehicles
        self._update_existing_tracks(detections, detection_metadata)
        
        # Add new detections as new tracks
        self._add_new_tracks(detections, detection_metadata)
    ...
    def _update_existing_tracks(self, detections: np.ndarray, detection_metadata: Optional[List[Dict]] = None):
        ...
        for vehicle_id, vehicle in self.tracked_vehicles.items():
            ...
            if best_detection_idx is not None:
                ...
                # Update detection metadata if available
                if detection_metadata and best_detection_idx < len(detection_metadata):
                    meta = detection_metadata[best_detection_idx]
                    vehicle.class_id = meta.get('class_id')
                    vehicle.class_name = meta.get('class_name')
                    vehicle.confidence = meta.get('confidence', 1.0)
    ...
    def _add_new_tracks(self, detections: np.ndarray, detection_metadata: Optional[List[Dict]] = None):
        ...
        for idx, detection in enumerate(detections):
            if idx not in matched_indices:
                ...
                # Get detection metadata if available
                class_id = None
                class_name = None
                confidence = 1.0
                if detection_metadata and idx < len(detection_metadata):
